...
现在不用RPC了，RPC主要解决不同服务间信息交换问题，而微服务中RESTful服务之间直接用HTTP+JSON交流。所以再言RPC框架就没有实际意义了。
...
Apache Dubbo (incubating) |ˈdʌbəʊ| 是一款高性能、轻量级的开源Java RPC 框架

|       DUBBO提供的功能       |    DUBBO功能实现    | SPRING CLOUD对应的解决方案 |
| :-------------------------: | :-----------------: | :------------------------: |
| 面向接口代理的高性能RPC调用 | JDK动态代理 + CGLIB |   RESTful : HTTP + JSON    |
|          智能容错           |                     |                            |
|          负载均衡           |                     |        Zuul Ribbon         |
|     服务自动注册和发现      |                     |           Eureka           |
...
- 缺省只对第一个参数 Hash，如果要修改，请配置 `<dubbo:parameterkey="hash.arguments"value="0,1"/>`
- 缺省用 160 份虚拟节点，如果要修改，请配置 `<dubbo:parameterkey="hash.nodes"value="320"/>`
